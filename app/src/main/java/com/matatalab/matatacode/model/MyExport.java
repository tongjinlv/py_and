...
public class MyExport{
    ...
    public MyExport(BluetoothGatt gatt,String text ) {


    }

    public byte[] parseRunCodeToBtCommand(String commandStr) {
        MLog.td("tjl", "parseRunCodeToBtCommand --- commandStr = " + commandStr);
        mCommandBuilder.clear();
        if (TextUtils.isEmpty(commandStr) == true) {
            MLog.td("tjl", "parseRunCodeToBtCommand --- message is empty!");
            return null;
        }
        ...
        if ((msgArray == null) || (msgArray.length == 0)) {
            MLog.td("tjl", "parseRunCodeToBtCommand --- msgArray is empty!");
            return null;
        }

        for (int i = 0; i < msgArray.length; i++) {
            MLog.td("tjl", "parseRunCodeToBtCommand --- msgArray[" + i + "] = " + msgArray[i]);
        }
        ...
        switch (msgArray[0]) {
            ...
            case "optimizeright":
                ...
                adjustmentTurn(CommandParse.Dircetion.right, angle, step);
                break;
            ...
            case "optimizeleft":
                ...
                adjustmentTurn(CommandParse.Dircetion.left, angle, step);
                break;
        ...
        MLog.td("tjl", "parseRunCodeToBtCommand --- bytes = "+ Global.hexToString(bytes));
        return bytes;
    }
...
//        }
    ...
    private void setBothEye(String color_str) {
        MLog.td("tjl", "setBothEye --- color_str = " + color_str);
        //使用单指令的工厂模式，待协议更新后适配
        ...
        MLog.td("tjl", "setBothEye --- color_ar = "+ color_ar);
    }
    ...
    private void setBotEye(int which, int color, int level) {
        ...
        switch (which) {
            case 1:
                mCommandBuilder.add((byte) 11);break;
            case 2:
                mCommandBuilder.add((byte) 5);break;
            case 3:
                mCommandBuilder.add((byte) 6);break;
            default:
                mCommandBuilder.add((byte) 11);break;
        }
    }
