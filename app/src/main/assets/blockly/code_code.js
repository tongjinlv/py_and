...
Code.toHexString = function(text) {
    let temp;
    let r = '';
    for (let val of text) {
...
Code.discard = function() {
    ...
    var blocks_cur = Code.workspace.getAllBlocks(true)
    ...
    //     return
    // } else {
    //     Blockly.Xml.clearWorkspaceAndLoadFromXml(Blockly.Xml.textToDom('<xml><block type="main_block" deletable="false" movable="false" x="20" y="20"></block></xml>'), Code.workspace);
    // }
    // 原逻辑，2个块以上会询问是否清理
    // if (count < 3 ||
...
document.write('<script src="msg_code/' + Code.LANG + '.js"></script>\n');
...
Code.SetLanguage = function(newLang) {
...
Code.setWorkSpaceMainBlock = function() {
    Blockly.Events.getGroup(true);
    Blockly.Xml.domToWorkspace(Blockly.Xml.textToDom('<xml><block type="main_block" deletable="false" editable="false" movable="false" x="20" y="20"></block></xml>'), Code.workspace);
    Blockly.Events.getGroup(false);
    Code.workspace.clearUndo();
}
Code.setBotBlockEnable = function() {

}
Code.setBotBlockDisable = function() {

}
Code.setControllerBlockEnable = function() {

}
Code.setWorkSpace = function(blocks_xml) {
    ...
    Blockly.Xml.domToWorkspace(blocks_res, Code.workspace);
}
Code.setWorkSpaceInitWithProject = function(blocks_xml) {
    Blockly.Events.getGroup(true);
    var blocks = Blockly.Xml.textToDom(blocks_xml);
    Blockly.Xml.domToWorkspace(blocks, Code.workspace);
    Blockly.Events.getGroup(false);
    Code.workspace.clearUndo();
}
